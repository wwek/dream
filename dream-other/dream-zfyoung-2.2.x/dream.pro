...
contains(QT,gui) {
    ...
    QT += network xml widgets webenginewidgets #multimedia
...
android {
    ...
    QT -= webkitwidgets
    QT += svg multimedia androidextras
...
win32 {
  ...
  win32-g++ {
	...
	DEFINES += NOMINMAX
        QMAKE_LFLAGS_RELEASE += /NODEFAULTLIB:libcmt.lib legacy_stdio_definitions.lib
        QMAKE_LFLAGS_DEBUG += /NODEFAULTLIB:libcmtd.lib legacy_stdio_definitions.lib
        QMAKE_LFLAGS_DEBUG += /NODEFAULTLIB:libcmt.lib legacy_stdio_definitions.lib
        LIBS += -lzlib -llibfftwf3-3
  }
  mxe {
    message('MXE')
...
speexdsp {
     HEADERS += src/resample/speexresampler.h
     SOURCES +=  src/resample/speexresampler.cpp
     DEFINES += HAVE_SPEEX
     LIBS += -lspeexdsp
