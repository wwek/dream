...
CWriteData::SetSoundInterface(string device)
{
	...
	if(pAudioOutput == nullptr) //handle default device
	{

		cerr << "can't find out device other than default one:"<< device<<endl;

		const QAudioDeviceInfo di = QAudioDeviceInfo::defaultOutputDevice();
    		if (!di.isFormatSupported(format)) {
        		cerr << "Default format not supported, trying to use the nearest."<<endl;
        		format = di.nearestFormat(format);
    		}
...
mutexSound.Lock();
    ...
    bool bBad = true;
    if((pAudioOutput != nullptr) && (pIODevice != nullptr))
    {
		...
		// dropping audio due to buffer is full/overflow, we absolutely 
		//don't want to be blocked in this routine!
	...
	if(n != 0)
	{
		cerr<<"audio output overflow, dropped "<< n <<"bytes"<<endl;
	}
...
//	Note: usaully periodSize() is 1/5 of bufferSize();
