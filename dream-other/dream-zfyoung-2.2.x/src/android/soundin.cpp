...
void COpenSLESIn::Enumerate(vector<string>& names, vector<string>& descriptions, string& defaultDevice)
{
...
static const int CHANNELS = 1; // If not 1 need to set the SLDataFormat_PCM channel mask correctly
...
result = (*recorderBufferQueue)->Enqueue(recorderBufferQueue, &record_buffers[i][0], record_buffers[i].size()*sizeof(short));
assert(SL_RESULT_SUCCESS == result);
}
