...
void CRx::OnAudioInputNotify()
{

    rx.GetReceiveData()->OnAudioInputNotify();
...
void CRx::LoadSettings()
{
    ...
    QAudioInput* pAudioInput = rx.GetAudioInput();	

    if(pAudioInput != nullptr)	
        connect(pAudioInput, SIGNAL(notify()), this, SLOT(OnAudioInputNotify()));
