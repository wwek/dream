...
main(int argc, char **argv)
{
	...
	(void)WSAStartup(MAKEWORD(2,2), &wsaData);
            ...
            try
            {
...
#ifdef USE_CONSOLEIO
                // Start status broadcast service for console mode
                string strStatusSocket = Settings.Get("command", "status-socket", string());
                CStatusBroadcast statusBroadcast;
                if (statusBroadcast.Start(&DRMReceiver, strStatusSocket))
                {
                    fprintf(stderr, "DRM status available at: %s\n",
                            statusBroadcast.GetSocketPath().c_str());
                }
                ...
                CConsoleIO::Leave();
#endif
                // StatusBroadcast will be stopped automatically by destructor
            }
